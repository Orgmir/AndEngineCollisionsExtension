...
public class BitmapPixelPerfectMask implements IPixelPerfectMask {
  ...
  public BitmapPixelPerfectMask(Bitmap bitmap, float pTextureX, float pTextureY, float pTextureWidth,
      float pTextureHeight, int pAlphaThreshold) {
    ...
    if (pAlphaThreshold < 0 || pAlphaThreshold > 255)
      throw new IllegalArgumentException("pAlphaThreshold should be in [0,255] range. " + pAlphaThreshold
          + " provided.");
  ...
  public boolean isSolid(final int pX, final int pY) {
    if (0 <= pX && pX < mBitsBlock.length && 0 <= pY && pY < mBitsBlock[0].length)
